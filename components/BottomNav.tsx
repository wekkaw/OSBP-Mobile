...
const NavItem: React.FC<{
  ...
  return (
    ...
    >
      ...
      <span className={`text-xs mt-1 transition-all duration-300 font-medium ${isActive ? 'opacity-100 text-indigo-500 dark:text-indigo-400' : 'opacity-0'}`}>
        {label}
      </span>
    </button>
  );
...
const BottomNav: React.FC<BottomNavProps> = ({ activeScreen, setActiveScreen }) => {
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 max-w-lg mx-auto h-20 bg-white/50 dark:bg-slate-900/50 backdrop-blur-xl border-t border-white/20 dark:border-slate-800/20">
      <div className="flex justify-around items-stretch h-full relative">
        <div 
          className="absolute top-0 h-full w-1/5 flex justify-center items-center transition-transform duration-300 ease-in-out"
          style={{ transform: `translateX(${activeIndex * 100}%)` }}
        >
          <div className="h-12 w-16 bg-gradient-to-br from-indigo-50 to-purple-50 dark:from-indigo-900/50 dark:to-purple-900/50 rounded-2xl"></div>
        </div>
        {navItems.map((item) => (
          <div key={item.label} className="flex-1 z-10">
            <NavItem
              label={item.label}
              icon={item.icon}
